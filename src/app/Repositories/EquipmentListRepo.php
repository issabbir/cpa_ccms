...
use App\Entities\Ccms\VendorType;
...
class EquipmentListRepo
{
     ...
     * @return mixed
     */
    public function findAllWithDistinctEquipmentName() {
        //Todo: Applied filters option as you need
        return $this->equipment->groupBy('equipment_name')
    ...
    public function findAllEquipmentList() {
        //Todo: Applied filters option as you need
        return $this->equipment->where('serial_no', '!=', null)
            ->get();
    }
     ...
     * @return mixed
     */
    public function findEquipmentNameWithoutVariants() {
        //Todo: Applied filters option as you need
        return $this->equipment->groupBy('cims.item_inventory_details.item_name','cims.item_inventory_details.item_id')
            ->leftJoin('cims.item_inventory_details', 'cims.item_inventory_details.id', '=', 'ccms.equipment_list.inventory_details_id')
            ->select('cims.item_inventory_details.item_name','cims.item_inventory_details.item_id')
            ->where('cims.item_inventory_details.item_name', '!=', null)
//            ->where('cims.item_inventory_details.department_id', '=', 10)
            ->get();
    }
...
// and el.vendor_no = vl.vendor_no");
    ...
    public function getEquipment() {
        return $this->equipment->leftJoin('cims.item_inventory_details', 'cims.item_inventory_details.id', '=', 'ccms.equipment_list.inventory_details_id')
            ->select('ccms.equipment_list.*','cims.item_inventory_details.item_name','cims.item_inventory_details.item_id');
    }
