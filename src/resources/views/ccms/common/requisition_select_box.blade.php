...
if (old('emp_id') || ($data && $data->requisition_for)) {

    if (old('emp_id'))
        $employee = DB::selectOne('select emp_id, emp_name from pmis.employee where emp_id = :emp_id', ['emp_id' => old('emp_id')]);
    else
        $employee = DB::selectOne('select emp_id, emp_name from pmis.employee where emp_id = :emp_id', ['emp_id' => $data->requisition_for]);
}
@endphp
